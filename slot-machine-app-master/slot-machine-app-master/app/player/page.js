...
const SlotMachine = () => {
  ...
  const [error, setError] = useState("");//invalid student number
  ...
  //updates the player's outcome on databsae
  const updateGameResult = async (outcome) => {
    try {
      const response = await fetch("https://localhost:7090/Player/UpdateGameResult", {
        method: "POST",
        headers: {
  ...
  const checkPlayEligibility = async () => {
    try {
      const res = await fetch(`https://localhost:7090/Player/can-play/${userName}`);
  
      if (!res.ok) {
        throw new Error("Server error or not found");
      }
  
      const text = await res.text();
      const canPlay = text ? JSON.parse(text) : false;
  ...
  const handleNameSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`https://localhost:7090/Player/validate/${userName}`);
      const isValid = await response.json();
  
      if (!isValid) {
        setError("Player does not Exist, please register first");
        return;
      }
      setError(""); 
      const eligible = await checkPlayEligibility();
      if (!eligible){
        setError("You have Already Played. Please try again after 3 hours");
        return;
      }
      setError("");
      ...
      setError("Error validating student number. Try again.");
    }

    try {
      const res = await fetch(`https://localhost:7090/Player/basic-info?studentNumber=${userName}`);
      const data = await res.json();
  
      if (data.length > 0) {
        const player = data[0]; // assuming one match
        setPlayerInfo(player);  // save in state
      } else {
  ...
  <           p className="error">{error}</p>
        ...
        // Game screen.
        <>
          <h1 className="text-4xl font-bold mb-6" style={{fontSize:50,color:"orange"}}>Slot Machine</h1>
          <h3 className="text-2xl font-bold mb-6" style={{ color: "silver" }}>Player {userName} : {playerInfo?.firstName} {playerInfo?.lastName} </h3>
